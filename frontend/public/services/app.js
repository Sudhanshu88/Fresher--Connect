(function () {
  ...
  function buildJobPostingStructuredData(job) {
    ...
    var schema = {
      ...
      hiringOrganization: {
        "@type": "Organization",
        name: job.company_name || "Fresher Connect",
        sameAs: job.company_website || window.location.origin + "/index.html",
  ...
  function companyLogoPlaceholder() {
    try {
      return new URL("components/fc-logo.svg", window.location.href).href;
    } catch (_error) {
      return "components/fc-logo.svg";
    }
  }

  function syncCompanyLogoPreview(logoUrl) {
    var preview = byId("companyLogoPreview");
    if (!preview) {
      return;
    }
    preview.src = logoUrl || companyLogoPlaceholder();
  }
  ...
  async function initCompanyPage() {
    ...
    if (byId("companyLogoForm")) {
      byId("companyLogoForm").addEventListener("submit", async function (event) {
        event.preventDefault();
        var form = event.currentTarget;
