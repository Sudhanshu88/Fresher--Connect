...
function CompanyWorkspace() {
  ...
  async function handleLogoUpload() {
    if (!logoFile) {
      setTone("error");
      setMessage("Choose a logo file before uploading.");
      return;
    }
  ...
  async function handleCreateJob(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setTone("error");
      setMessage("Job posting failed. Check required fields.");
    } finally {
  ...
  if (loading && !dashboard) {
    return <LoadingBlock label="Loading company workspace..." />;
  }
  ...
  return (
    <div className="stack">
      ...
      <section className="detail-grid">
        ...
        <section className="panel stack">
          ...
          <div className="stats-grid compact-grid">
            ...
            <StatCard label="Avg decision" value={`${dashboard.analytics.avg_decision_hours}h`} />
          </div>
          <div className="detail-list">
        ...
        <form className="form" onSubmit={handleCreateJob}>
          <div className="two-col">
            <label className="field">
              <span>Title</span>
              <input value={jobForm.title} onChange={(event) => setJobForm((current) => ({ ...current, title: event.target.value }))} required />
            </label>
            <label className="field">
              <span>Department</span>
              <input value={jobForm.department} onChange={(event) => setJobForm((current) => ({ ...current, department: event.target.value }))} required />
            </label>
          </div>
          <label className="field">
            <span>Description</span>
            <textarea value={jobForm.description} onChange={(event) => setJobForm((current) => ({ ...current, description: event.target.value }))} required />
          </label>
          <div className="two-col">
            <label className="field">
              <span>Experience required</span>
              <input value={jobForm.experience_required} onChange={(event) => setJobForm((current) => ({ ...current, experience_required: event.target.value }))} required />
            </label>
            <label className="field">
              <span>Education required</span>
              <input value={jobForm.education_required} onChange={(event) => setJobForm((current) => ({ ...current, education_required: event.target.value }))} required />
            </label>
          </div>
          <div className="two-col">
            <label className="field">
          ...
          <div className="two-col">
            ...
            <label className="field">
              <span>Location</span>
              <input value={jobForm.location} onChange={(event) => setJobForm((current) => ({ ...current, location: event.target.value }))} />
            </label>
          </div>
          <div className="two-col">
            <label className="field">
          ...
          <label className="field">
            ...
            <textarea value={jobForm.role_overview} onChange={(event) => setJobForm((current) => ({ ...current, role_overview: event.target.value }))} />
          </label>
          <label className="field">
            <span>Benefits</span>
            <textarea value={jobForm.benefits} onChange={(event) => setJobForm((current) => ({ ...current, benefits: event.target.value }))} />
          </label>
      ...
      <section className="detail-grid">
        <section className="panel stack">
          <span className="section-label">Posted jobs</span>
          {!dashboard.posted_jobs.length ? <div className="empty">No jobs posted yet.</div> : null}
          <div className="list">
            {dashboard.posted_jobs.map((job) => (
              <article className="activity-card stack" key={job.id}>
                ...
                <div className="meta">
                  {job.location || "Flexible"} | {job.work_mode || "-"} | {job.application_count || 0} applicants
                </div>
                <div className="row">
        ...
        <section className="panel stack">
          <span className="section-label">Top jobs</span>
          {!dashboard.analytics.top_jobs.length ? <div className="empty">No analytics data yet.</div> : null}
          <div className="list">
      ...
      <section className="panel stack">
        ...
        {!dashboard.recent_activity.length ? <div className="empty">No audit activity yet.</div> : null}
        <div className="list">
